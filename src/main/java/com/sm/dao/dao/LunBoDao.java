...
public class LunBoDao {
    ...
    public LunBoInfo getById(Integer id) {
        final String sql = String.format("select id, image, link_type,link_id from %s where id =?", VarProperties.LUNBO);
        List<LunBoInfo> lunBoInfo = jdbcTemplate.query(sql, new Object[]{id}, new LunBoInfo.LunBoInfoRowMapper());
        ...
        return lunBoInfo == null || lunBoInfo.isEmpty() ? null: lunBoInfo.get(0);
    }
}