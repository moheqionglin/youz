...
public class ReceiveAddressManagerDao {
    ...
    public void add(ReceiveAddressManager receiveAddressManager) {
        final String sql = String.format("insert into %s(address_name, address_detail,send_pay) values(?,?,?)", VarProperties.RECEIVE_ADDRESS_MANAGER);
        jdbcTemplate.update(sql,  new Object[]{receiveAddressManager.getAddressName(), receiveAddressManager.getAddressDetail(),0});
    }

    public void update(ReceiveAddressManager receiveAddressManager) {
        final String sql = String.format("update %s set address_name = ? , address_detail = ? where id = ?", VarProperties.RECEIVE_ADDRESS_MANAGER);
        jdbcTemplate.update(sql, new Object[]{receiveAddressManager.getAddressName(), receiveAddressManager.getAddressDetail(), receiveAddressManager.getId()});
    }
    ...
    public List<ReceiveAddressManager> queryAddressList() {
        String sql = String.format("select id, address_name , address_detail " +
